...
| **Benchmark task assert regexes break** — benchmark tasks assert output about paths | None | `claudeDir` still appears as a property name inside `ClaudePaths` in the refactored file; any LLM output describing the file will still emit that token. The `claude` key at the top of `AppPaths` also matches the `verbose-paths` assert. No change to `tasks.ts` needed. |
| **`analyticsBufferDir` inconsistency** — left in `AppPaths` while other tracker-related paths move | Intentional | It lives at `~/agent-analytics` (not `~/.claude`), is Atlas-owned (written by Atlas hooks, not Claude Code), and was explicitly excluded from the user's extraction list. Keeping it in `AppPaths` is the correct call. |

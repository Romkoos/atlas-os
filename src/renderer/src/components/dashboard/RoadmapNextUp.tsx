...
export function RoadmapNextUp() {
  ...
  const count = (status: RoadmapStatus): number => rows.filter((i) => i.status === status).length
  ...
  return (
    <div className="panel">
      ...
      <div className="panel-body nextup-body">
        {items.isLoading ? (
          <Note>loading…</Note>
        ) : total === 0 ? (
          ...
          <>
            ...
            {/* Full-board tally, pinned to the bottom so the panel reads as one
                complete unit at the galaxy's height. */}
            <button type="button" className="nextup-tally" onClick={openBoard}>
              <span>
                <b>{count('in-progress')}</b> wip
              </span>
              <span>
                <b>{count('planned') + count('todo')}</b> queued
              </span>
              <span>
                <b>{count('done')}</b> done
              </span>

...
describe('groupNextUp', () => {
  ...
  it('caps groups (4 in progress, 6 next up, 4 done) and sorts wip/done by recency', () => {
    const wip = [1, 2, 3, 4, 5].map((n) =>
      item({ status: 'in-progress', updatedAt: n, title: `w${n}` }),
    )
    const next = [1, 2, 3, 4, 5, 6, 7, 8].map((n) => item({ status: 'todo', title: `n${n}` }))
    const done = [1, 2, 3, 4, 5].map((n) => item({ status: 'done', updatedAt: n, title: `d${n}` }))
    const g = groupNextUp([...wip, ...next, ...done])
    expect(g.inProgress.map((i) => i.title)).toEqual(['w5', 'w4', 'w3', 'w2'])
    expect(g.nextUp).toHaveLength(6)
    expect(g.done.map((i) => i.title)).toEqual(['d5', 'd4', 'd3', 'd2'])
  })

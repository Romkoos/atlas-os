...
export interface RunTrendingOptions {
  model: string
  onToken: (text: string) => void
}

// Tools the github-trending skill needs end-to-end: invoke the skill, fetch the
// trending page, get the date via Bash, and write the digest file itself. With an
...
const TRENDING_PROMPT =
  'Run the github-trending skill and compile a digest of the top 10 trending GitHub repositories this week. ' +
...
// excluded from the Knowledge page (see knowledge/store EXCLUDED). Both feeds —
// ai-news and github-trending — share this folder.
export function newsDir(): string {
  return join(storeRoot(), 'news')
}

export function trendingFilePath(): string {
  return join(newsDir(), 'github-trending.md')
}

// Read the single overwritten digest. Absent file → empty raw + null mtime so the
// UI can show its empty state without throwing.
export function readTrending(): { raw: string; updatedAt: string | null } {
  const file = trendingFilePath()
...
// Pro/Max OAuth (mirrors claude.ts / benchmark/runner.ts / news.ts).
...
// Run the github-trending skill headlessly, streaming the model's text output. The
// skill owns the file write; `done` resolves with the path once the run succeeds.
export function runTrending(opts: RunTrendingOptions): TrendingRun {
  ...
  const done = (async (): Promise<{ filePath: string; outputTokens: number }> => {
    ...
    const q = query({
      ...
      options: {
        model: opts.model,
        settingSources: ['user'], // load ~/.claude/skills so github-trending is available
        allowedTools: TRENDING_TOOLS,
    ...
    for await (const message of q as AsyncIterable<SDKMessage>) {
      if (message.type === 'stream_event') {
        ...
        if (message.subtype !== 'success') {
          ...
          throw new Error(`Trending run failed: ${reason}`)
        }
      }
    }

    return { filePath: trendingFilePath(), outputTokens }
  })()
